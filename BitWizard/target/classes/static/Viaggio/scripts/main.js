...
async function fetchViaggiConAmici() {
	try {
		const response = await fetch('api/amicizia/trovaViaggiConAmici', {
			method: 'GET',
			credentials: 'include',
			headers: {
				'Content-Type': 'application/json'
		...
		// Filtrare i viaggi unici
		const viaggiUnici = Array.from(new Set(viaggi.map(viaggio => viaggio.viaggioId)))
			.map(id => viaggi.find(viaggio => viaggio.viaggioId === id));

		displayViaggiConsigliati(viaggiUnici);
	} catch (error) {
		console.error('Errore durante il recupero dei viaggi:', error);
	}
}

function displayViaggiConsigliati(viaggi) {
	const consigliatiDiv = document.getElementById('consigliati');
	consigliatiDiv.innerHTML = '';

	if (viaggi.length === 0) {
		consigliatiDiv.innerHTML = '<p>Nessun viaggio consigliato.</p>';
		return;
	}

	viaggi.forEach(viaggio => {
		const viaggioElement = document.createElement('div');
		viaggioElement.classList.add('viaggio-consigliato');
		viaggioElement.innerHTML = `
            <h5>${viaggio.nome}</h5>
            <p>Luogo di Partenza: ${viaggio.luogoPartenza}</p>
            <p>Luogo di Arrivo: ${viaggio.luogoArrivo}</p>
            <p>Data di Partenza: ${new Date(viaggio.dataPartenza).toLocaleDateString()}</p>
            <p>Data di Ritorno: ${new Date(viaggio.dataRitorno).toLocaleDateString()}</p>
            <p>Prezzo: €${viaggio.prezzo.toFixed(2)}</p>
            <a href="/paginaViaggio/viaggio?viaggioId=${viaggio.viaggioId}" class="btn btn-primary">Dettagli del viaggio</a>
        `;
		consigliatiDiv.appendChild(viaggioElement);
	});
